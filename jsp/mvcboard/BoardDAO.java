...
public class BoardDAO {
	...
	public boolean boardInsert(BoardBean board) {
		...
		try {
			...
			sql = "insert into board_2 (board_num, board_name, board_pass, board_subject, board_content, board_file, "
	...
	public int getListCount(String cond) {
		...
		try {
			...
			// pstmt = con.prepareStatement("select count(*) for baord_2")
	...
	public BoardBean getDetail(int num) {
		...
		try {
			...
			System.out.println("getDtail() 에러 : " + e);
		}finally {
			try {
				if (rs != null)
					rs.close();
	...
	public boolean getBoardWriter(int num, String pass) {
		String sql = "select board_pass from board_2 where board_num = ?";
		System.out.println("getBoardWriter - String pass로 들어온것 : " + pass);
		try {
			con = ds.getConnection();
			...
			System.out.println("getBoardWriter() 글쓴사람 아님 에러 : " + e);
		}finally {
			try {
				if (rs != null)
					rs.close();
	...
	public boolean delete(int num) {
		...
		String sql = "delete from board_2 where BOARD_NUM=?";
	...
	public int boardReply(BoardBean board) {
			...
			try {
				...
				pstmt.setInt(2,re_seq);	// re_req : 관련 글 출력 순서
				result = pstmt.executeUpdate();
